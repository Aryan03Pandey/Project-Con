...
import serviceAccountKey from "./thoughts-b92d1-firebase-adminsdk-nikn6-6c869b3867.json" assert {type: 'json'};
...
const verify = (req, res, next) => {
    //access token is provided by the frontend as a part of request header with the key - 'authorization'
    /*
    headers: {
    ...
    .then((user) => {
        ...
        if(!user.google_auth){
            bcrypt.compare(password, user.personal_info.password, (err, result) => {
                if(err){
                    return res.status(403).json({ "error": "Error occured while trying to log in. Plase try again" })
                }
...
server.post("/google-auth", async (req, res) => {
    ...
    .then(async (decodedUser) => {
        ...
        if(user){ //if the email id already exists
            if(!user.google_auth){ //if the user had signed up using email and password, don't let him login using google
        ...
        else{//the email is new, so sign up the user
...
server.post("/change-password", verify, (req, res) => {
    ...
    .then((user) => {
        ...
        if(user.google_auth){
            return res.status(403).json({ error: "Cannot change password as the User is logged in through Google" })
        }

        bcrypt.compare(currentPassword, user.personal_info.password, (err, result) => {
            if(err){
                return res.status(500).json({ error: 'Error in changing the Password. Please try again later' })
            }
...
server.post("/update-profile-img", verify, (req, res) => {
...
server.post("/update-profile", verify, (req, res) => {
    ...
    if(bio.length > bioLimit){
        return res.status(403).json({ error: "Bio cannot be more than " + bioLimit + " characters long" })
    }
...
server.post("/create-blog", verify, (req, res) => {
    ...
    if(!title.length){
        return res.status(403).json({ error: "Provide a blog title" });
    }

    if(!draft){
        if(!des.length || des.length > 200){
            return res.status(403).json({ error: "Provide a blog description under 200 characters" });
        }
    
        if(!banner.length){
            return res.status(403).json({ error: "Provide a banner for the blog" });
        }
    
        if(!content.blocks.length){
            return res.status(403).json({ error: "Cannot publish an empty blog" });
        }
    
        if(!tags.length || tags.length > 10){
            return res.status(403).json({ error: "Provide a maximum of 10 tags to the blog" });
        }
    }
...
server.post("/like-blog", verify, (req, res) => {
...
server.post("/isLiked", verify, (req, res) => {
    let user_id = req.user;
...
server.post("/add-comment", verify, (req, res) => {
    ...
    new Comment(commentObj).save().then(async commentFile => {
        ...
        new Notification(notification).save().then(notification => console.log("New notification created"))
...
server.post("/delete-comment", verify ,(req, res) => {
    ...
    .then(comment => {
        if(user_id == comment.commented_by || user_id == comment.blog_author){
            ...
            return res.status(403).json({ error: "Don't have permission to delete this comment" })
        }
    })

})

server.get("/new-notification", verify, (req, res) => {
...
server.post('/notifications', verify, (req, res) => {
    let user_id = req.user;
...
server.post("/all-notifications-count", verify, (req, res) => {
    let user_id = req.user;
...
server.post("/user-written-blogs", verify, (req, res) => {
...
server.post("/user-written-blogs-count", verify, (req, res) => {
...
server.post("/delete-blog", verify, (req, res) => {
